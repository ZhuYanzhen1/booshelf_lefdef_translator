...
SclParser::SclParser(const std::string &input_file_name) {
    ...
    scl_para = file_contents.substr(space_loc + 2);
}

void SclParser::parse(SclDatabase &scl_db) {
    size_t space_loc = scl_para.find("\n\n");
    std::string front_line = scl_para.substr(0, space_loc);
    scl_db.get_value().num_rows = parsing_number_(front_line);
    scl_para = scl_para.substr(space_loc + 2);

    int counter_int = 0;
    std::stringstream counter_stream(scl_para);
